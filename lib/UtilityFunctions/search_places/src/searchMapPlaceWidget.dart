...
class _SearchMapPlaceWidgetState extends State<SearchMapPlaceWidget>
  ...
  void initState() {
    ...
    //var Size= MediaQuery.of(widget.contextTop).size.height*0.8;
    _animationController =
        AnimationController(vsync: this, duration: Duration(milliseconds: 500));
    _containerHeight = Tween<double>(begin: 55, end: 400).animate(
      CurvedAnimation(
  ...
  */
  ...
  Widget _placeOption(Place prediction) {
    List<String> place = prediction.description.split(",");
    String title = place[0];
    String description = "";
    place.asMap().forEach((index, element) {
      if (index != 0) {
    ...
    print("getPrediction >> $description");

    return Container(
      padding: EdgeInsets.symmetric(horizontal: 5, vertical: 3),
      child: ListTile(
        minLeadingWidth: 20,
        onTap: () => _selectPlace(prediction: prediction),
        leading: Container(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(
              "${description}",
              //description.length < 45 ?: "${description.replaceRange(45, place.length, "")} ..."
              style: TextStyle(
                fontSize: MediaQuery.of(context).size.width * 0.04,
                color: widget.darkMode ? Colors.grey[100] : Colors.grey[850],
              ),
